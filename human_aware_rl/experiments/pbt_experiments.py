...
def evaluate_pbt_for_layout(layout_name, num_rounds, pbt_performance, pbt_model_paths, best_test_bc_models, seeds,
                            best=False):
    ...
    assert common_keys_equal(bc_params["mdp_params"],
                             pbt_config["mdp_params"]), "Mdp params differed between PBT and BC models training"
    assert common_keys_equal(bc_params["env_params"],
                             pbt_config["env_params"]), "Env params differed between PBT and BC models training"

    pbt_agents = [get_pbt_agent_from_config(pbt_save_dir, pbt_config["sim_threads"], seed=seed, agent_idx=0, best=best)
                  for seed in seeds]
...
def eval_pbt_over_seeds(pbt_agents, bc_agent, layout_name, num_rounds, pbt_performance, agent_evaluator):
    ...
    for i in range(len(pbt_agents)):
        pbt_and_pbt = ae.evaluate_agent_pair(AgentPair(pbt_agents[i], pbt_agents[i], allow_duplicate_agents=True),
...
if __name__ == "__main__":
    ...
    run_all_pbt_experiments(best_bc_model_paths)
