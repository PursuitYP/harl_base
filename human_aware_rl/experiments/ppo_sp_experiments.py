...
def plot_ppo_sp_training_curves(ppo_sp_model_paths, seeds, single=False, show=False, save=False):
    for layout, model_path in ppo_sp_model_paths.items():
        ...
        if save:
            plt.savefig("rew_ppo_sp_" + layout, bbox_inches='tight')
        if show:
            plt.show()
...
def evaluate_sp_ppo_and_bc(layout, ppo_sp_path, bc_test_path, num_rounds, seeds, best=False, display=False):
    ...
    for seed in seeds:
        ...
        ppo_and_ppo = evaluator.evaluate_agent_pair(AgentPair(agent_ppo, agent_ppo, allow_duplicate_agents=True),
        ...
        ppo_and_bc = evaluator.evaluate_agent_pair(AgentPair(agent_ppo, agent_bc_test), num_games=num_rounds,
                                                   display=display)
        avg_ppo_and_bc = np.mean(ppo_and_bc['ep_returns'])
        sp_ppo_performance[layout]["PPO_SP+BC_test_0"].append(avg_ppo_and_bc)

        bc_and_ppo = evaluator.evaluate_agent_pair(AgentPair(agent_bc_test, agent_ppo), num_games=num_rounds,
                                                   display=display)
...
def evaluate_all_sp_ppo_models(ppo_sp_model_paths, bc_test_model_paths, num_rounds, seeds, best):
    ...
    for layout in ppo_sp_model_paths.keys():
        print(layout)
        layout_eval = evaluate_sp_ppo_and_bc(layout, ppo_sp_model_paths[layout], bc_test_model_paths[layout],
                                             num_rounds, seeds, best)
...
def run_all_ppo_sp_experiments(best_bc_model_paths):
    ...
    set_global_seed(124)
    num_rounds = 100
    ppo_sp_performance = evaluate_all_sp_ppo_models(ppo_sp_model_paths, best_bc_model_paths['test'], num_rounds, seeds,
                                                    best=True)
