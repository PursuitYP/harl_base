...
def evaluate_all_ppo_hm_models(ppo_hm_model_paths, bc_test_model_paths, num_rounds, seeds, best):
    ...
    for layout in ppo_hm_model_paths.keys():
        print(layout)
        layout_eval = evaluate_ppo_hm_and_bc(layout, ppo_hm_model_paths[layout], bc_test_model_paths[layout],
...
def run_all_ppo_hm_experiments(best_bc_model_paths):
    ...
    set_global_seed(124)
    num_rounds = 50
    ppo_hm_performance = evaluate_all_ppo_hm_models(ppo_hm_model_paths, best_bc_model_paths['test'], num_rounds, seeds,
                                                    best=True)
